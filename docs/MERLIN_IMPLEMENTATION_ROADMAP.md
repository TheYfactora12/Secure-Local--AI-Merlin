...
Next work should be selected as a narrow slice: close or split #1 based on the release policy decision, then continue #53 under v2.0. The follow-on Phase 3 learning sequence is documented in `docs/MERLIN_PHASE3_LEARNING_PLAN.md`: outcome observer first, then retrieval scoring, preference extraction, session reflection, and skill scores.
