...
| Offline launch | Wi-Fi-off restart from installed runtime | local services start or degrade honestly | Network-disconnected core restart passed with Wi-Fi off and `--pull never`; all endpoints returned `200` by probe 3; no cloud endpoint hits or pull/download entries were found; see `docs/release/evidence/2026-05-12-network-disconnected-launch-validation.md` | Pass with model-readiness gap | No chat-capable local model installed; user currently sees terminal-only model install guidance |
