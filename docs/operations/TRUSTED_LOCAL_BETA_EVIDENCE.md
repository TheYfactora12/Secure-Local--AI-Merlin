...
| Offline launch | Wi-Fi-off restart from installed runtime | local services start or degrade honestly | Network-disconnected core restart passed with Wi-Fi off and `--pull never`; all endpoints returned `200` by probe 3; no cloud endpoint hits or pull/download entries were found; no-model UX was polished to point users to Brains before manual setup; see `docs/release/evidence/2026-05-12-network-disconnected-launch-validation.md` and `docs/release/evidence/2026-05-12-model-readiness-ux-polish.md` | Pass with explicit model-install boundary | No chat-capable local model installed; offline chat still requires an explicitly installed local model |
