...
The current roadmap item is v3.1 product proof: #135 Rooms, #123 offline local brain/context store, #134 product value checkpoint, #106 Wizard HQ shell, #130 brain storage location, and #31/#32/#120 memory approval/review/delete. Recent commits added the default local Rooms layout, session Room launcher, approval-gated transcript save, approval-gated Room Master Prompt draft generation, and Round Table suggest-only governance. The next recommended slice is the Wizard HQ Room Master Prompt review/edit surface followed by a separate approve-for-context gate. Do not start native automation, broad connectors, ClosClaw web comprehension, or Developer ID signing until the local chat/Rooms/memory loop is clearer.
