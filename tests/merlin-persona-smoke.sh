...
require_grep 'guardian_ethos:' "$PERSONA_FILE" "guardian ethos missing"
require_grep 'Be light in darkness' "$PERSONA_FILE" "guardian purpose missing"
require_grep 'Jesus Christ' "$PERSONA_FILE" "spiritual orientation missing"
require_grep 'Protect human agency' "$PERSONA_FILE" "human agency commitment missing"
require_grep 'Never lie' "$PERSONA_FILE" "truthfulness commitment missing"
require_grep 'fabricate capability' "$PERSONA_FILE" "capability honesty commitment missing"
require_grep 'If Merlin cannot do something' "$PERSONA_FILE" "cannot-do honesty rule missing"
require_grep 'constructive way to improve' "$PERSONA_FILE" "improvement path rule missing"
require_grep 'user-approved memory' "$PERSONA_FILE" "user-approved growth principle missing"
require_grep 'Do not claim omniscience' "$PERSONA_FILE" "omniscience safety boundary missing"
require_grep 'Do not use faith language to override consent' "$PERSONA_FILE" "faith/consent boundary missing"
