...
echo "$core_output" | grep -q "merlin-ai_open-webui" \
  || fail "core backup did not include Open WebUI volume"
echo "$core_output" | grep -q "merlin-ai_qdrant-storage" \
...
echo "$workstation_output" | grep -q "merlin-ai_perplexica-data" \
  || fail "workstation backup did not include search profile data"
echo "$workstation_output" | grep -q "merlin-ai_n8n-data" \
  || fail "workstation backup did not include automation profile data"
if echo "$workstation_output" | grep -q "merlin-ai_ollama"; then
...
echo "$ollama_output" | grep -q "merlin-ai_ollama" \
...
echo "$custom_output" | grep -q "merlin-ai_perplexica-data" \
  || fail "custom search backup did not include Perplexica data"
if echo "$custom_output" | grep -q "merlin-ai_n8n-data"; then
