...
require_output "$PLAN_OUTPUT" '^mode: plan_only$' "Magic Mode should state plan-only mode"
require_output "$PLAN_OUTPUT" '^boundary: planning_only_no_execution$' "Magic Mode boundary missing"
...
require_output "$PLAN_OUTPUT" '^tools_required: .*shell' "code plan should show tools required"
require_output "$PLAN_OUTPUT" '^pause_guidance: Pause means review this plan; no action is running\.$' "pause guidance missing"
require_output "$PLAN_OUTPUT" '^stop_guidance: Stop means discard the plan; there is no background execution to stop\.$' "stop guidance missing"
...
grep -q '"mode":"plan_only"' "$PLAN_LOG" || fail "plan log should record plan-only mode"
grep -q '"pause_supported":true' "$PLAN_LOG" || fail "plan log should record pause support"
grep -q '"stop_supported":true' "$PLAN_LOG" || fail "plan log should record stop support"
grep -q '"tools_required":' "$PLAN_LOG" || fail "plan log should record required tools"
